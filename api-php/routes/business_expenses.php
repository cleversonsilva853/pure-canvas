...
if ($method === 'PUT' && $id) { $body = getJsonBody(); $fields = []; $params = []; foreach (['name','category','amount','date','observation'] as $f) { if (array_key_exists($f, $body)) { $fields[] = "`$f` = ?"; $params[] = $body[$f]; } } $params[] = $id; $db->prepare('UPDATE business_expenses SET ' . implode(', ', $fields) . ' WHERE id = ?')->execute($params); $stmt2 = $db->prepare('SELECT * FROM business_expenses WHERE id = ?'); $stmt2->execute([$id]); jsonResponse($stmt2->fetch()); }
