...
if ($method === 'PUT' && $id) { $body = getJsonBody(); $fields = []; $params = []; foreach (['name','sale_price','cost_price','stock'] as $f) { if (array_key_exists($f, $body)) { $fields[] = "`$f` = ?"; $params[] = $body[$f]; } } $params[] = $id; $db->prepare('UPDATE business_products SET ' . implode(', ', $fields) . ' WHERE id = ?')->execute($params); $stmt2 = $db->prepare('SELECT * FROM business_products WHERE id = ?'); $stmt2->execute([$id]); jsonResponse($stmt2->fetch()); }
